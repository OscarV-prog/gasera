...
export const operationsRouter = {
   ...
   */
  listReturnLoads: protectedProcedure
    .input(
      z.object({
    ...
    .query(async ({ ctx, input }) => {
      ...
      // Enforce Supervisor role manually to allow graceful fallback for no-tenant users
      const role = ctx.session.user.role;
      if (!["superadmin", "admin", "supervisor"].includes(role ?? "")) {
        // If user has a tenant but wrong role, throw.
        // But if we want to be nice, maybe just return empty?
        // Standard is throw FORBIDDEN for wrong permissions.
        throw new TRPCError({
          code: "FORBIDDEN",
  ...
  getReturnLoad: supervisorProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const [load] = await db
        ...
        .from(returnLoads)
        .where(
          and(eq(returnLoads.id, input.id), eq(returnLoads.tenantId, tenantId)),
        )
        .limit(1);
  ...
  listDiscrepancies: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      const tenantId = ctx.session.user.tenantId;
      if (!tenantId) return { discrepancies: [], nextCursor: undefined };
   ...
   */
  listBillingRequests: protectedProcedure
    .input(
      z.object({
        limit: z.number().min(1).max(100).default(50),
        cursor: z.string().nullish(),
    ...
    .query(async ({ ctx, input }) => {
      const tenantId = ctx.session.user.tenantId;
      if (!tenantId) return { requests: [], nextCursor: undefined };

      const role = ctx.session.user.role;
      // Allow customers to see their own requests
      if (
        !["superadmin", "admin", "supervisor", "cliente"].includes(role ?? "")
      ) {
        throw new TRPCError({
          code: "FORBIDDEN",
      ...
      // If user is a client, force filter by their linked customerId
      if (role === "cliente") {
        ...
        conditions.push(eq(billingRequests.customerId, linkedCustomer.id));
      } else if (customerId) {
   ...
   */
  createBillingRequest: protectedProcedure
    .input(
      z.object({
        customerId: z.string(),
        orderId: z.string().optional(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // If client, force customerId from link
      if (role === "cliente") {
        const userEmail = ctx.session.user.email;
        if (!userEmail)
          throw new TRPCError({
